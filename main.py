...
list_N = input(f'Enter N, Ai: ').split()
...
def nearval(array, number):
    return min(array, key=lambda x: abs(number - abs(x))) 

print(f'To the number {x} nearest ->{nearval(array, x)}')
...
N = abs(int(input('Enter 1 = Eng, Enter 0 = Rus: ')))
